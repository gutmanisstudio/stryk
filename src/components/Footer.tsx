"use client";

import Image from "next/image";
import Link from "next/link";
import FadeIn from "./FadeIn";

export default function Footer() {
  return (
    <footer className="border-t border-neutral-800 bg-neutral-950">
      <div className="mx-auto max-w-7xl px-5 py-12 sm:px-6 sm:py-16">
        <div className="grid gap-10 sm:grid-cols-2 sm:gap-12 lg:grid-cols-4">
          {/* Brand */}
          <FadeIn>
            <div>
              <Link href="/" className="flex items-center gap-3">
                <Image
                  src="/images/logo.PNG"
                  alt="STRYK"
                  width={44}
                  height={44}
                  className="rounded-md"
                />
                <span className="text-lg font-bold uppercase tracking-tighter text-white">
                  STRYK
                </span>
              </Link>
              <p className="mt-4 text-sm leading-relaxed text-neutral-500">
                Premium sports nutrition built on transparency, clean formulas,
                and real performance. Fuel the fight within.
              </p>
            </div>
          </FadeIn>

          {/* Navigation */}
          <FadeIn delay={0.1}>
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-[0.2em] text-neutral-400">
                Navigation
              </h4>
              <ul className="mt-5 space-y-3">
                {[
                  { label: "Shop", href: "/shop" },
                  { label: "About", href: "/#about" },
                  { label: "Athletes", href: "/#athletes" },
                  { label: "FAQ", href: "/#faq" },
                  { label: "Contact", href: "/contact" },
                ].map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-sm text-neutral-500 transition-colors hover:text-white"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </FadeIn>

          {/* Contact */}
          <FadeIn delay={0.2}>
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-[0.2em] text-neutral-400">
                Contact
              </h4>
              <ul className="mt-5 space-y-3">
                <li>
                  <a
                    href="tel:27717545"
                    className="text-sm text-neutral-500 transition-colors hover:text-white"
                  >
                    27717545
                  </a>
                </li>
                <li>
                  <a
                    href="https://wa.me/27717545"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-sm text-green-500 transition-colors hover:text-green-400"
                  >
                    <svg
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      className="h-4 w-4"
                    >
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                    </svg>
                    WhatsApp
                  </a>
                </li>
              </ul>
            </div>
          </FadeIn>

          {/* Products */}
          <FadeIn delay={0.3}>
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-[0.2em] text-neutral-400">
                Products
              </h4>
              <ul className="mt-5 space-y-3">
                {[
                  { label: "Electrolytes", href: "/product/electrolytes" },
                  { label: "BCAA 2:1:1", href: "/product/bcaa" },
                  { label: "Shakers", href: "/shop" },
                ].map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-sm text-neutral-500 transition-colors hover:text-white"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </FadeIn>
        </div>

        {/* Bottom bar */}
        <FadeIn>
          <div className="mt-16 border-t border-neutral-800 pt-8">
            <p className="text-center text-xs text-neutral-600">
              &copy; {new Date().getFullYear()} STRYK Nutrition. All rights
              reserved.
            </p>
          </div>
        </FadeIn>
      </div>
    </footer>
  );
}
